...
namespace APIV2.Mark.Business.Services
{
    public class CustomerService :ICustomerService
    {
        ...
        public CustomerService(UtilitiyContext context, IAccountService account)
        {
            _context = context;
            _account = account;
        }

        public async Task<ApiResponse<bool>> Create(Customer customer)
        {
            ...
            try
            {
                ...
                if (customer != null)
                {
                    ...
                    {
                        AccountId = 11,
                        AccountType = "Account Receivables",
                        Balance = customer.Balance,
                        EmpId = customer.EmpId,
                        Code = customer.Code,
                        CreationDate = DateTime.Now,
                        Description = "Current Assets",
                        NameAr = customer.NameEn,
        ...
        public async Task<ApiResponse<bool>> Edit(Customer customer)
        {
            var result = new ApiResponse<bool>();
            try
            {
                ...
                var account = await _account.GetItem(customer.Code);
